...
import { drillCount, GRAMMAR_SHARE } from '../lib/grammar';
            ...
            <div class="kv"><span class="k">{S.gram.share}</span>
              <div class="pills">
                <button class={'pill ' + ((s.grammarShare ?? GRAMMAR_SHARE) === 0 ? 'on' : '')}
                  onClick={() => upd(m => (m.settings.grammarShare = 0))}>{S.gram.shareOff}</button>
                {[15, 25, 40].map(n => (
                  <button key={n} class={'pill ' + ((s.grammarShare ?? GRAMMAR_SHARE) === n ? 'on' : '')}
                    onClick={() => upd(m => (m.settings.grammarShare = n))}>{n} %</button>
                ))}
              </div>
            </div>
            <div class="tiny" style="margin:2px 0 6px">
